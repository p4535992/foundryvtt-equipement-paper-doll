...
import {
    ...
    inventoryItemClass,
    inventorySlot,
} from "../constants/objectClassNames.js";
import {
    addBoxComponent,
    emptyInventorySlot,
    inventoryItem,
...
export default class PaperDollApp extends FormApplication {
     ...
     */
    extractDataFromForm() {
        const divStructureArray = [
            ...document.querySelectorAll(primaryItems),
    ...
    removeElementAndSecondaries(element, itemToRemove) {
        ...
        const isShadowItem =
            ...
            element.id === `${itemToRemove.id}${shadowItemModifier}` &&
            element.className === addedItemClass;
    ...
    unEquipItem([item]) {
        ...
        flagForItem?.split?.(", ")?.forEach?.((slot) => {
            ...
            this.slotStructure[slotName] = this.slotStructure[slotName].map((slot) =>
                slot === itemId || slot === `${itemId}${shadowItemModifier}` ? "" : slot,
            );
        });
    ...
    addSearchEvent([html]) {
        ...
        addBoxes.forEach((box) =>
            box.addEventListener("click", (source) => {
        ...
        tabs.forEach((tab) => {
            ...
            tab.addEventListener("click", () => {
                tabContents.forEach((tabContent) => {
                    if (tabContent.getAttribute("data-tab") === tabTarget) tabContent.classList.remove(closedTabClass);
    ...
    replaceInventorySlotsWithItems([html]) {
        ...
        slots.forEach((slot, index) => {
            ...
            const itemElement = createHTMLElement(
                inventoryItem(itemId, item.img, item.system.equipped, item, reverted),
            );
