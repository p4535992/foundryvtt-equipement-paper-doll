...
export default class itemSearchApp extends FormApplication {
    ...
    getAvailableSlotsAtLocation(location) {
        return [...document.querySelectorAll(availableSlots(location))].filter((item) => item !== this.source.target);
    }
    ...
    findEquippedItems() {
        const IDs = [];
        document.querySelectorAll(allEquippedItems).forEach((item) => IDs.push(item.id));
        return IDs;
    }
